...
#define IF_NOT_CONN_NUM(index)  if ((index < 0) || (index >= GSM_GPRS_SERVICE_MAX)) return -1;
...
typedef int (* pFun_GPRS_disconnected)(void *data, int socket);
typedef int (* pFun_GPRS_received)(void *data, uint32_t len);
typedef int (* pFun_GPRS_rejected)(void *data);
typedef int (* pFun_GPRS_closed)(void *data);
typedef int (* pFun_GPRS_accepted)(void *data);
typedef int (* pFun_GPRS_report_IP)(uint8_t *local_IP, uint16_t local_port, uint8_t *remote_IP, uint16_t remote_port);
...
#define THREAD_GSM_MG301_STACK_SIZE     2048
extern rt_uint8_t prio_GSM_MG301;
extern rt_thread_t thread_GSM_MG301;

#define THREAD_GSM_RX_STACK_SIZE        2048
