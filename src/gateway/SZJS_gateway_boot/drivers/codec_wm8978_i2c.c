...
/* MCLK : 16M (PLLPRESCALE) (divide by 2 sets the required) */
#define PLL_N_16000000            (11 | PLLPRESCALE)
#define PLL_K_16000000            (4194304)

/* CODEC port define */
#define CODEC_I2S_PORT          SPI2
#define CODEC_I2S_RCC           RCC_APB1Periph_SPI2
...
void vol(uint16_t v) // 0~99
{
    ...
    codec_send(REG_LOUT1_VOL | vol);
    codec_send(REG_ROUT1_VOL | HPVU | vol);
    codec_send(REG_LOUT2_VOL | vol);
    codec_send(REG_ROUT2_VOL | SPKVU | vol);
}

int8_t vol_up(uint8_t step)
{
    if ((codec.vol_out + step) <= 99)
    {
        codec.vol_out += step;
        vol(codec.vol_out);
        return 0;
...
int8_t vol_down(uint8_t step)
{
    if (codec.vol_out >= step)
    {
        codec.vol_out -= step;
        vol(codec.vol_out);
        return 0;
...
rt_err_t codec_hw_init(const char * i2c_bus_device_name)
{
    ...
    codec.vol_out = CODEC_VOL_DEFAULT;
    codec.vol_in  = 0;
