			   ...
			   struct port_options *ops)
{
	...
	if (addr < 0x03 || addr > 0x77) {
		rt_kprintf( "I2C address out of range [0x03-0x77]\n");
		return PORT_ERR_UNKNOWN;
	}

	/* 3. open it */
	h = rt_calloc(sizeof(*h), 1);
	if (h == NULL) {
		rt_kprintf( "End of memory\n");
		return PORT_ERR_UNKNOWN;
	}
	fd = open(ops->device, O_RDWR);
	if (fd < 0) {
		rt_kprintf( "Unable to open special file \"%s\"\n",
			ops->device);
		rt_free(h);
		return PORT_ERR_UNKNOWN;
	}
	...
	if (ret < 0) {
		rt_kprintf( "I2C ioctl(funcs) error %d\n", errno);
		close(fd);
		rt_free(h);
		return PORT_ERR_UNKNOWN;
	}
	if ((funcs & I2C_FUNC_I2C) == 0) {
		rt_kprintf( "Error: controller is not I2C, only SMBUS.\n");
		close(fd);
		rt_free(h);
		return PORT_ERR_UNKNOWN;
	}
	...
	if (ret < 0) {
		rt_kprintf( "I2C ioctl(slave) error %d\n", errno);
		close(fd);
		rt_free(h);
		return PORT_ERR_UNKNOWN;
	}
