...
void rt_thread_entry_sys_IAP(void* parameter)
{
    ...
    #if 0  // Just for program ID to OTP area manually. default is 0.
    
    volatile uint64_t board_ID = 102163300024L;
    uint32_t OTP_ID_1 = board_ID%0x0100000000;
    uint32_t OTP_ID_2 = board_ID/0x0100000000;


    FLASH_If_Write((uint32_t *)(SYS_ID_ADDRESS-16), (uint32_t *)&board_ID, 8);
    //FLASH_If_Write(SYS_ID_ADDRESS-16+4, &OTP_ID_2, 4);
    
////    FLASH_If_Write(SYS_ID_ADDRESS, &OTP_ID_1, 4);
////    FLASH_If_Write(SYS_ID_ADDRESS+4, &OTP_ID_2, 4);
