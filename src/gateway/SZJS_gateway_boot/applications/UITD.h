...
#define GB_ROUGH_BUF_MAX		(GB_ROUGH_LEN_MAX * 4)
...
#define GB_TYPE_128_ALIVE			        128
#define GB_TYPE_129_D_PUSH_FW_INFO 	        129
#define GB_TYPE_130_U_REQ_FW_DATA           130
#define GB_TYPE_131_D_ACK_FW_DATA           131
#define GB_TYPE_132_U_REQ_FW_INFO           132
#define GB_TYPE_133_D_ACK_FW_INFO           133
#define GB_TYPE_134_U_REQ_FILE_INFO         134
#define GB_TYPE_135_D_ACK_FILE_INFO         135
#define GB_TYPE_136_D_PUSH_FORCE_FW_INFO    136
...
typedef int (* pFun_srv_report_IP)(uint8_t *local_IP, uint16_t local_port, uint8_t *remote_IP, uint16_t remote_port);
typedef int (* pFun_srv_connected)(void *data);
typedef int (* pFun_srv_accepted)(void *data);

typedef int (* pFun_srv_disconnect)(int socket);
typedef int (* pFun_srv_disconnected)(void *data, int socket);

typedef int (* pFun_srv_connect_fail)(void *data);

typedef int (* pFun_srv_received)(void *data, uint32_t len);
typedef int (* pFun_srv_rejected)(void *data);
typedef int (* pFun_srv_closed)(void *data);

typedef int (* pFun_srv_connect)(uint8_t if_UDP, uint8_t if_use_name, uint8_t *IP, uint16_t port, 
...
extern int UITD_sensor_upload_rough_data(t_server_handler *handler, uint8_t ID, uint8_t out_type, uint16_t data);
