...
//    rt_kprintf("Software version: %s\n", SW_VER);
    sys_ID_read(g_UITD_addr, 0, sizeof(g_UITD_addr));
    ...
    g_UITD_SN = 0;
    rt_memcpy(&g_UITD_SN, g_UITD_addr, sizeof(g_UITD_addr));
    ...
    ADC_ext_board_module_check(&sys_config.dev_type, &sys_config.module_type);

    sys_config.board_type_checked = 1;
    
    if ((sys_config.dev_type != dev_type_sample_ctrl) && (sys_config.dev_type != dev_type_controller))
    {
        ADC_power_battery_init();
    }
...
void  rt_UITD_GPRS_init(void *parameter)
{
    ...
    mode_thread_start(&mode_cfg.thread);
...
//    }
    ...
    else
    {
        ...
        else if (sys_config.dev_type == dev_type_simple_UITD)
        {
        ...
        else if (sys_config.dev_type == dev_type_controller)
        {
             //------- create controller input thread
            ...
            {        
             ...
             //------- create controller output thread
        ...
        else if (sys_config.dev_type == dev_type_UITD_3uart)
        {
             //------- create fire_alarm thread
             thread_fire_alarm = rt_thread_create("FA",
                                                rt_thread_entry_fire_alarm,
