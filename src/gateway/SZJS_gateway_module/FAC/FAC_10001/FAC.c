...
static uint8_t  parse_reset_flag = 0;
...
int QB_5Ei_parse(s_com_bus_cb * cb, uint8_t *data, uint32_t len)
{
		int i = 0;
		...
		cmd_status = data[0] & data[1];
		addr_station = data[12] & data[13];
		addr_line = data[14] & data[15];
		dev_ID = data[16] & data[17];
...
//						rt_memcpy(&cb->alarm_fire.remark[18], (uint8_t *)&cmd_status, 4);
		  	...
		  	FA_mq_fire_2(&cb->alarm_fire, sizeof(s_com_bus_R_alarm));
				...
				SYS_log(SYS_DEBUG_INFO, ("ALARM :  %d - %d - %d \n", addr_station, addr_line, dev_ID));
		  	
		  }
		  else if (cmd_status == 4) // fault_alarm
		  {
						cb->alarm_fault.valid = 1;
...
//						rt_memcpy(&cb->alarm_fault.remark[18], (uint8_t *)&cmd_status, 4);
		  	...
		  	FA_mq_fault_2(&cb->alarm_fault, sizeof(s_com_bus_R_alarm));
				...
				SYS_log(SYS_DEBUG_INFO, ("FAULT :  %d - %d - %d \n", addr_station, addr_line, dev_ID));
		  }
...
int FAC_unusual_rx_handler(s_com_bus_cb * cb)
{
    ...
    if (cb->parse.valid)
    {
        ...
        res = QB_5Ei_parse(cb, cb->parse.buf, cb->parse.len);
