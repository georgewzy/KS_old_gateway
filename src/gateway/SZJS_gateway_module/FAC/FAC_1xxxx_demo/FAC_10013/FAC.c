...
#define FAC_MSG_HEAD		0xFE
#define FAC_MSG_TAIL		0xFF

#define FAC_MSG_CMD_ALARM			1
#define FAC_MSG_CMD_FAULT			2
#define FAC_MSG_CMD_RESET			0
...
int FAC_msg_parse(s_com_bus_cb * cb, uint8_t *data, uint32_t len)
{
		...
		else if ((data[2] == FAC_MSG_CMD_ALARM) || (data[2] == FAC_MSG_CMD_FAULT))
		{
		
			if (data[2] == FAC_MSG_CMD_ALARM)
			{
				FAC_msg_type = FAC_MSG_TYPE_ALARM;
			}
			else if (data[2] == FAC_MSG_CMD_FAULT)
			{
			...
			for (i=0;i<data_meta_num;i++)
			{
				...
				if (FAC_msg_type == FAC_MSG_TYPE_ALARM)
				{
						...
						cb->alarm_fire.addr_sub  = dev_ID + addr_line*0x0100;
				...
				else if (FAC_msg_type == FAC_MSG_TYPE_FAULT)
				{
						...
						cb->alarm_fault.addr_sub  = dev_ID + addr_line*0x0100;
