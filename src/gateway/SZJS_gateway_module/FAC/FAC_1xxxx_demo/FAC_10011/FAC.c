...
int FAC_msg_parse(s_com_bus_cb * cb, uint8_t *data, uint32_t len)
{
  ...
  if (FAC_msg_type == FAC_MSG_TYPE_ALARM)
  {
						...
						cb->alarm_fire.addr_main = data[2];
						cb->alarm_fire.addr_sub  = data[4]*0x100 + data[3];
						...
						cb->alarm_fire.dev_info.device_ID = (uint32_t)data[2]*0x10000 + (uint32_t)data[3]*0x100 + (uint32_t)data[4];
  ...
  else if (FAC_msg_type == FAC_MSG_TYPE_FAULT)
  {
						...
						cb->alarm_fault.addr_main = data[2];
						cb->alarm_fault.addr_sub  = data[4]*0x100 + data[3];
						...
						cb->alarm_fault.dev_info.device_ID = (uint32_t)data[2]*0x10000 + (uint32_t)data[3]*0x100 + (uint32_t)data[4];
