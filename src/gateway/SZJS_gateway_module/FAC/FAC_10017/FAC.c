...
#define FAC_MSG_HEAD		0x23
#define FAC_MSG_TAIL		0x23

#define FAC_MSG_CMD_ALARM			0x01
#define FAC_MSG_CMD_RESET			0x83

#define FAC_MSG_CMD_FAULT			0x04
...
int FAC_msg_parse(s_com_bus_cb * cb, uint8_t *data, uint32_t len)
{
		...
		//vTemp = 0;
		//vNum  = 0;
		addr_area = ( (data[13]&0x0F)<<4 ) | (data[14]&0x0F);
		sys_addr = addr_area;
		addr_line = ( (data[15]&0x0F)<<4 ) | (data[16]&0x0F);
		dev_ID = ( (data[17]&0x0F)<<4 ) | (data[18]&0x0F);
		
		vCmd = ( (data[1]&0x0F)<<4 ) | (data[2]&0x0F);
		...
		*/
		...
		else if (vCmd == FAC_MSG_CMD_ALARM)
		{
			FAC_msg_type = FAC_MSG_TYPE_ALARM;
		}
		else if (vCmd == FAC_MSG_CMD_FAULT)				//π ’œ
		{
