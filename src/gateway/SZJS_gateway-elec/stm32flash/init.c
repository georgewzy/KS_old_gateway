...
static int write_to(const char *filename, const char *value)
{
	...
	if (fd < 0) {
		rt_kprintf( "Cannot open file \"%s\"\n", filename);
		return 0;
	}
	ret = write(fd, value, strlen(value));
	if (ret < 0) {
		rt_kprintf( "Error writing in file \"%s\"\n", filename);
		close(fd);
		return 0;
	}
	close(fd);
...
{
	rt_kprintf( "GPIO control only available in Linux\n");
	return 0;
}
#else
	...
	do {
		...
		if (ret < 0) {
			if (errno == EINTR || errno == EAGAIN || errno == EWOULDBLOCK)
				continue; /* try again */
			rt_kprintf( "Error reading in file \"%s\"\n", filename);
			close(fd);
			return 0;
		}
		n += ret;
...
static int drive_gpio(int n, int level, struct gpio_list **gpio_to_release)
{
	...
	if (ret) {
		...
		ret = stat(file, &buf);
		if (ret) {
			rt_kprintf( "GPIO %d not available\n", n);
			return 0;
		}
		exported = 0;
	}
	...
	if (exported == 0 || input == 1) {
		new = (struct gpio_list *)rt_malloc(sizeof(struct gpio_list));
		if (new == NULL) {
...
//			l -= 3;
			...
			rt_kprintf( "Character \'%c\' is not a digit\n", *s);
			ret = 0;
			break;
		}
		...
		if (*s && (l > 0)) {
			if (*s == ',') {
				...
				rt_kprintf( "Character \'%c\' is not a separator\n", *s);
				ret = 0;
				break;
			}
		}
