...
#include <finsh.h>
int ISP_flash(char *filename, uint8_t verify)
{
    strcpy(ISP_opt.filename, filename);
    ISP_opt.action = ACT_WRITE;
    ISP_opt.verify = verify;
    ISP_opt.reset_flag = 1;
    
    ISP_stm32(&port_opts, &ISP_opt);
}
FINSH_FUNCTION_EXPORT_ALIAS(ISP_flash, ISP_flash, ISP flash for stm32 --filename  --verify.);
...
//					rt_kprintf(	"Invalid baud rate, valid options are:\n");
...
//					rt_kprintf( "Invalid serial mode\n");
//					return 1;
//				}
...
//					rt_kprintf( "ERROR: Invalid options, can't specify start page / num pages and start address/length\n");
//					return 1;
//				}
//				npages = strtoul(optarg, NULL, 0);
//				if (npages > 0xFF || npages < 0) {
//					rt_kprintf( "ERROR: You need to specify a page count between 0 and 255");
//					return 1;
//				}
//				if (!npages)
//					no_erase = 1;
...
//				if (execute % 4 != 0) {
//					rt_kprintf( "ERROR: Execution address must be word-aligned\n");
//					return 1;
//				}
//				break;
//			case 's':
...
//						if (readwrite_len == 0) {
//							rt_kprintf( "ERROR: Invalid options, can't specify zero length\n");
//							return 1;
//						}
//					}
//				}
...
//					rt_kprintf( "ERROR: Invalid negative value for option -F\n");
//					return 1;
//				}
//				if (port_opts.rx_frame_max == 0)
...
//					rt_kprintf( "ERROR: current code cannot work with small frames.\n");
//					rt_kprintf( "min(RX) = 20, min(TX) = 6\n");
//					return 1;
//				}
//				if (port_opts.rx_frame_max > STM32_MAX_RX_FRAME) {
//					rt_kprintf( "WARNING: Ignore RX length in option -F\n");
//					port_opts.rx_frame_max = STM32_MAX_RX_FRAME;
//				}
//				if (port_opts.tx_frame_max > STM32_MAX_TX_FRAME) {
//					rt_kprintf( "WARNING: Ignore TX length in option -F\n");
//					port_opts.tx_frame_max = STM32_MAX_TX_FRAME;
//				}
//				break;
//			case 'f':
...
//		if (port_opts.device) {
//			rt_kprintf( "ERROR: Invalid parameter specified\n");
//			show_help(argv[0]);
//			return 1;
//		}
...
//		rt_kprintf( "ERROR: Device not specified\n");
//		show_help(argv[0]);
//		return 1;
//	}

//	if ((action != ACT_WRITE) && verify) {
//		rt_kprintf( "ERROR: Invalid usage, -v is only valid when writing\n");
//		show_help(argv[0]);
//		return 1;
//	}
