...
int gpio_pin_circle(int pin, int connect, int disconnect, int times)
{
    int i = 0;
    ...
    for (i=0;i<times;i++)
    {
        ...
        rt_thread_delay(RT_TICK_PER_SECOND * connect);
        rt_pin_write(pin, PIN_LOW);
        rt_thread_delay(RT_TICK_PER_SECOND * disconnect);
    }
    
    return 0;
}
FINSH_FUNCTION_EXPORT_ALIAS(gpio_pin_circle, pin_circle, Pin output 0_1 circly : --pin  --connect  --disconnect  --times.);
