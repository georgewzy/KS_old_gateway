...
int GSM_srv_init(s_GSM_CB *cb, rt_device_t dev)
{
    if (cb == NULL) return -1;
...
int GSM_srv_phaser(void)
{
    ...
    while(1)
    {
        ...
        if ( (data_temp == ',') &&
            (p_GSM_CB->rec_len > sizeof("^SISR:") + 3)
            )
        {
...
int GSM_srv_send_data(uint8_t *data, uint32_t len)
{
    ...
    if (len && (data != NULL))
    {
        ...
        res = rt_sem_take(&sem_GSM_rx_data, RT_TICK_PER_SECOND / 2);
                            ...
                            if (p != NULL) 
                            {
                                ...
                                if (p != NULL) 
                                {
                                    ...
                                    if (p_GSM_CB->local_port)
                                    {
                                        ...
                                        {
                                            p_GSM_CB->service[p_GSM_CB->SISO_index]->report_IP(p_GSM_CB->local_IP, p_GSM_CB->local_port, NULL, 0);
                                        }
                                    }
                                }
                            }
...
int GSM_srv_rec_handler(void)
{
    ...
    GSM_PRINTF("Rec : %s", p_GSM_CB->rec_buf);
        ...
        case GSM_term_off:
            switch (p_GSM_CB->status.term_off)
            {
                ...
                case GSM_term_off_power_off:
                    ...
                    GSM_PRINTF("GSM module power off !\n");
...
s_GPRS_service* GPRS_srv_create(s_GPRS_srv_cfg *cfg)
{
...
int GPRS_connect(s_GPRS_srv_cfg *cfg,
                ...
                )
{
    ...
    gprs_service = GPRS_srv_create(cfg);
...
//int GPRS_SZJS_connected(void *data)
//{
//    GSM_PRINTF("SZJS server connected! \n");
//    
//    
//}
...
//int GPRS_SZJS_received(void *data, uint32_t len)
//{
//    int i = 0;
//    uint8_t *p = data;
//    
...
//int GPRS_SZJS_disconnect(void)
//{
//    if ((con_SZJS >= 0) && (con_SZJS < GSM_GPRS_SERVICE_MAX))
//    {
//        if (p_GSM_CB->service[con_SZJS]->status == GPRS_up)
//        {
//            p_GSM_CB->service[con_SZJS]->status = GPRS_release;
//        }
//    }
//}

//int GPRS_SZJS_disconnected(void *data)
//{
//    //GPRS_srv_destroy(con_SZJS);
//    //GPRS_srv_release(con_SZJS);
//}


//int GPRS_SZJS_connect(void)
//{
//    int res = 0;
//    
//    
//    res = GPRS_connect(&GPRS_cfg_SZJS, NULL, &GPRS_SZJS_connected, &GPRS_SZJS_disconnected, &GPRS_SZJS_received, NULL, NULL);
//    if (res < 0)
//    {
//        GSM_PRINTF("GPRS_connect error: %d\n", res);
//    }
//    
//    con_SZJS = res;
//    GSM_PRINTF("SZJS server connected , service ID: %d\n", con_SZJS);
//    
//    return res;
//}

//int GPRS_SZJS_write(uint8_t *data)
//{
//    GPRS_srv_write(con_SZJS, data, strlen(data));
//    return 0;
//}
...
//int GPRS_XFZD_connected(void *data)
//{
//    GSM_PRINTF("XFZD server connected! \n");
//    
//    
//}
...
//int GPRS_XFZD_received(void *data, uint32_t len)
//{
//    int i = 0;
//    uint8_t *p = data;
//    
...
//int GPRS_XFZD_disconnect(void)
//{
//    if ((con_XFZD >= 0) && (con_XFZD < GSM_GPRS_SERVICE_MAX))
//    {
//        if (p_GSM_CB->service[con_XFZD]->status == GPRS_up)
//        {
//            p_GSM_CB->service[con_XFZD]->status = GPRS_release;
//        }
//    }
//}

//int GPRS_XFZD_disconnected(void *data)
//{

//}

//int GPRS_XFZD_rejected(void *data)
//{
//    GSM_PRINTF("XFZD connect has been rejected! \n");
//}

//int GPRS_XFZD_closed(void *data)
//{
//    GSM_PRINTF("XFZD connect has been closed! \n");
//}

////int GPRS_XFZD_connect(void)
////{
////    int res = 0;
////    
////    
////    res = GPRS_connect(&GPRS_cfg_XFZD, 
////                        NULL, 
////                        &GPRS_XFZD_connected, 
////                        &GPRS_XFZD_disconnected, 
////                        &GPRS_XFZD_received, 
////                        &GPRS_XFZD_rejected, 
////                        &GPRS_XFZD_closed,
////                        &GPRS_XFZD_report_IP
...
////    
////    con_XFZD = res;
////    GSM_PRINTF("XFZD server connected , service ID: %d\n", con_SZJS);
////    
////    return res;
////}

//int GPRS_XFZD_write(uint8_t *data)
//{
//    GPRS_srv_write(con_XFZD, data, strlen(data));
//    return 0;
//}
                ...
                case GPRS_up:
                    if (p_GSM_CB->ack.ack_stru.DATA_REC)
                    {
                        if (p_GSM_CB->data_handle == i)
                        {
                            ...
                            {
                                p_GSM_CB->service[i]->received(&p_GSM_CB->rec_buf[p_GSM_CB->rec_index], p_GSM_CB->data_len);
                            }
        ...
        if (p_GSM_CB->service[i] != NULL)
        {
            switch (p_GSM_CB->service[i]->status)
            {
                ...
                case GPRS_discon:
                    ...
                    {
                        p_GSM_CB->service[i]->disconnected(NULL, i);
                    }
                    GPRS_srv_destroy(i);
                    GPRS_srv_release(i);
                    break;
...
//        {
//            rt_kprintf("Sem_take  GSM_rx  failed !\n");
//            rt_thread_delay(5);  
//            rt_kprintf("Sem_take  GSM_rx  over !\n");
...
//    rt_sem_init(&sem_GSM_MG301_rx, "GSM_MG301_rx", 0, RT_IPC_FLAG_FIFO);
    ...
    GSM_srv_init(&GSM_CB, device_GSM_MG301);
