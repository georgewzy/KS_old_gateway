...
void rt_thread_entry_UITD_main(void* parameter)
{
    ...
    while(sys_config.board_type_checked == 0)			//等待确定主板应用
    {
        rt_thread_delay(2);
    }
...
//    ISP_test("/boot/SZJS_UITD_sub.hex");
    ...
    while(1)
    {
        res = rt_sem_take(&sem_GSM_up, 1);		//等待GSM链接网络信号量
			
        if (res == RT_EOK)
        {
            
            if (sys_config.server_SZJS.svr_use_name)		//根据域名 链接服务器  
            {
            ...
            sys_config.server_main_connected = 1;			//标志服务器已经链接
            break;
        }
    }
    ...
    while(1)
    {
        ...
        {
            sys_status_set(&sys_ctrl, SYS_state_reconnect);			//状态指示
        ...
        if (sys_fw_update_reboot)			//系统固件更新重启系统
        {
