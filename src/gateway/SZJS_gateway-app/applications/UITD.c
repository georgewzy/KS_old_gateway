...
int UITD_file_trans_init(t_server_handler *handler, s_file_trans_cb *file_trans, uint8_t buf_num)
{
    ...
    if (handler->if_UITD == 0)
    {
        ...
        file_trans_init(handler->file_trans);  		//初始化数据缓冲区      
    }
    else
    {
...
int UITD_service_handler(t_server_handler *handler)
{
    ...
    if (handler->AP_ctrl.restart)			//AP重启 (网关重启)
    {
        handler->AP_ctrl.restart_cnt ++;
        if (handler->AP_ctrl.restart_cnt > RT_TICK_PER_SECOND*5) // waiting at least 5S before restart. 等待5S复位完成
        {
            handler->AP_ctrl.restart_cnt = 0;
            handler->AP_ctrl.restart = 0;
            // Restart the system.	AP重启
            sys_reboot();	
            while(1) rt_thread_delay(10);
        }
    }
    else if (handler->AP_ctrl.shutdown)		//AP关闭 (网关关闭)
    {
    ...
    {
        switch (handler->status)													//链接网络
        {
            case UITD_svc_status_idle:					//空闲状态
                handler->alive_cnt = 0;
                ...
                if (handler->if_UITD)
                {
                    UITD_send_UITD_inital_status(handler);		 //初始化状态进入激活状态 handler->status = UITD_svc_status_alive
                ...
                handler->status = UITD_svc_status_connect;	 			//链接状态
                #endif // UITD_SERVICE_TESTING
                break;
            case UITD_svc_status_connect:
            
                break;
            case UITD_svc_status_alive:														//服务器端链接状态激活
                if (handler->alive_cnt > SERVER_ALIVE_PERIOD)			
                {
                    UITD_send_alive_pkt(handler);									//发送心跳包
    ...
    //            }
            ...
            case UITD_svc_status_testing:
                ...
                #endif // UITD_SERVICE_TESTING
                break;
            case UITD_svc_status_disconnected:							//断开链接
                //handler->status = UITD_svc_status_idle;
                handler->status = UITD_svc_status_stoped;
...
//                }
        ...
        switch (handler->FW.status)														//--------写文件操作
        {
            case FW_idle:
...
//                }
    ...
    {
        res = rt_mq_recv(mq_sensor_send, &sensor_data_temp, sizeof(sensor_data_temp), 0);   //接收到传感器采样消息
        if (res == RT_EOK)
        {
        ...
        if (handler->output.valid)										//IO输出标志被置位
        {
            handler->output.valid = 0;
            
            for (i=0;i<handler->output.num;i++)
            {
                rt_mq_send(mq_output_ctrl, &handler->output.data[i], sizeof(handler->output.data[0]));		//发送控制消息
            }
        }
    }
