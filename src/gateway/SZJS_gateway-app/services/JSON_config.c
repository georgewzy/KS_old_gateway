...
eth_cfg_load_start:
    ...
    if (res >= 0)
    {
        ...
        if (JSON_item != NULL) 
            eth_cfg.if_DHCP = 1;//JSON_item->valueint;
...
mode_cfg_load_start:
    ...
    if (res >= 0)
    {
        ...
        if (JSON_item != NULL) 
            mode_cfg.GPRS = JSON_item->valueint;

        JSON_item = cJSON_GetObjectItem(pJSON, "ETHERNET");
        if (JSON_item != NULL) 
            mode_cfg.ETHERNET = JSON_item->valueint;
    ...
    if (res >= 0)
    {
        ...
        for (i=0;i<FA_COM_BUS_MAX;i++)
        {
            ...
            if (FA_en)
            {
                ...
                JSON_item = cJSON_GetObjectItem(pJSON, json_buf);
                if (JSON_item != NULL) 
                    p_cfg[j].FA_type = 0;JSON_item->valueint;
...
int json_cfg_load(void)				//系统配置参数读取
{
    int res = 0;
    cJSON *pJSON;
    cJSON *JSON_item ;
...
sys_cfg_load_start:
    ...
    if (res >= 0)
    {
        ...
        if (JSON_item != NULL) 
            sys_config.server_SZJS.svr_use_name = 1;//JSON_item->valueint;
