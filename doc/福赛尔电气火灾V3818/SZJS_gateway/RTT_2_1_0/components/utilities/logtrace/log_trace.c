...
RTM_EXPORT(log_trace); // Song: Added at 2017-3-22 11:15:51
...
RTM_EXPORT(log_trace_flush); // Song: Added at 2017-3-22 11:15:51
...
// Song: added 2016-10-11 16:49:41
void log_trace_list_date(void)
{
    size_t now;

    rt_time(&now);
    log_trace("%s\n", ctime(&now));
    log_trace("%lu * %d\n", now, RT_TICK_PER_SECOND);
    log_trace_flush();
}
FINSH_FUNCTION_EXPORT_ALIAS(log_trace_list_date, log_date, Print date to log);
...
FINSH_FUNCTION_EXPORT_ALIAS(log_trace_close, log_close, Stop log trace);
